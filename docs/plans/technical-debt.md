...
| TD-0006 | CodeQL `analyze` and Scorecard SARIF `upload-sarif` use `continue-on-error` on this private repo without GHAS; results are not stored to the GitHub Security tab. CodeQL runs on push-to-main + schedule only (not PR) because the empty template has no source to recognize and GHAS is unavailable. | `.github/workflows/codeql.yml`, `.github/workflows/scorecard.yml` | Open | Enable GitHub Advanced Security (paid) OR make the repo public (free Code Scanning) to activate full storage; then re-add `pull_request` to the CodeQL triggers. |
