...
assert_contains "PR title context matches emitted check" "-f 'required_status_checks\[contexts\]\[\]=PR Title Check / PR Title Check'"
assert_contains "metadata context matches emitted check" "-f 'required_status_checks\[contexts\]\[\]=validate-metadata / Validate required docs & metadata'"
assert_contains "docs context matches emitted check" "-f 'required_status_checks\[contexts\]\[\]=docs-check / Markdown lint \+ link check \+ TBD/TODO scan'"
assert_contains "actionlint context matches emitted check" "-f 'required_status_checks\[contexts\]\[\]=action-security / actionlint \(workflow syntax\)'"
assert_contains "zizmor context matches emitted check" "-f 'required_status_checks\[contexts\]\[\]=action-security / zizmor \(workflow security\)'"
