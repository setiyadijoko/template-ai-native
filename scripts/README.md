...
| `init-project.sh` | After creating a repository from this template | Requires an explicit `--profile starter\|standard\|enterprise` (never inferred), replaces the marked consumer project identity block in `README.md`, and writes credential-free `.template/project.yaml` and `.template/profile.yaml` together. `--project-type` defaults to `other` and `--ai-enabled` to `false`; Enterprise requires a non-`none` `--deployment-target`. Single/undecided layouts use version 1, while monorepos require repeatable `--component ID=PATH:STACK` options and use version 2. It does not configure workflows, source code, or credentials. Use `--reconfigure` for an intentional replacement. |
...
| `validate-profile-config.sh` | `make profile-config-check` / `make ci` | Validates the credential-free `.template/profile.yaml` maturity-profile contract. Missing config keeps template compatibility mode; an initialized consumer missing a profile fails the policy check. Profile values do not activate workflows. |
