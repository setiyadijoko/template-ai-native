...
| `validate-profile-config.sh` | `make profile-config-check` / `make ci` | Validates the optional, credential-free `.template/profile.yaml` maturity-profile contract. Missing config keeps compatibility mode; profile values do not activate workflows. |
| `resolve-profile-shadow.sh` | Called by the advisory profile-shadow workflow; run manually to inspect a profile | Validates the optional profile and normative mapping, emits hypothetical decisions, and never controls workflow execution. |
